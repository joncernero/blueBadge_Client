...
const SearchFlowers = (props) => {
   ...
   async function fetchFlowers(){
      ...
      const baseurl = 'https://trefle.io/api/v1/species?filter%5Bflower_color%5d=';
      const token = 'token=FKCFSL2qgSy2Gnwimlt25A-Ze2oYTp-CACmUCTxbtSc';
      const color = `${flowerColor}&`
      const pageurl = `&page=${page}`
        const url = `${baseurl}${color}${token}${pageurl}&order[common_name]=asc`;
        console.log(url);
  ...
  if(direction === 'up'){
    ...
    fetchFlowers();
        ...
        function displayCards(){
          return plants.length >0 ? plants.map((plant) => <DisplayFlowers plant={plant} token={props.token}/>) : null;
      }
...
<div>
  <div>
    ...
    <input type="text" name="flowersearch" onChange={(e) => setFlowerColor(e.target.value)}/>
